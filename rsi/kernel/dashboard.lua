...
<li>A candidate is retained only if its paired held-out gain is significant (bootstrap, &alpha;=0.05), it loses nothing on the regression suite, it does not drop on the fresh adversarial split, and its visible-split gain is not out of proportion to its held-out gain (overfit check).</li>
...
function render(s,p,lines){
  ...
  $('lib').innerHTML=(c.library||[]).map(e=>'<div><b class="acc">'+esc(e.name)+'</b> = '+esc(e.expr)+' <small>'+esc(e.arg)+'→'+esc(e.ret)+' &middot; '+esc(e.origin||'')+'</small></div>').join('')||'<small>empty: nothing learned yet</small>';
}
