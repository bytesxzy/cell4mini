...
* **Scores go up only when evidence says so.** Expect long stretches with no acceptance. Over the
  generations run during development, the harness rejected every candidate: the closest was a
  +1.5pp held-out gain at p=0.155. Nothing was accepted, so the champion never moved. A system that
  reported daily gains under this rule would be lying. If you want to watch the bar itself, the
  lineage table shows every candidate, its delta, its p-value and why it failed.
