...
-- Which ARC files a generation scores.
--
...
function M.run_generation(opts)
  ...
  -- Seeded on the generation so the window rotates across generations but is fixed within one:
  ...
  -- The external window rotates by generation while held-out and regression do not depend on it, so
