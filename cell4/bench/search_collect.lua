...
function M.solve(task, ctx)
  ...
  -- COLLECT VARIANT (measurement only, not for production):
  -- a program whose outputs equal the targets on every training example is "train-consistent".
  -- Production returns the FIRST such program and the OE dedup then hides every later one behind
  -- the same key. Here we exempt the target key, record every consistent program, and keep going.
  local function consider(ty, c, node, outs)
    ...
    local hit = back[key]
    if hit then COLLECT[#COLLECT + 1] = hit.build(node) return nil end
    local l = bucket(ty, c)
